#ifndef DOGBREEDS_POMERANIAN_SYNCOBSERVABLE_H
#define DOGBREEDS_POMERANIAN_SYNCOBSERVABLE_H
...
template <typename T>
class SyncObservable : public ObservableInterface<T> {
 private:
  T m_value;
 ...
 public:
  SyncObservable() {
    JackRussell::TopicManager::getInstance()
        .createSyncTopic<std::pair<std::shared_ptr<T>, std::shared_ptr<T>>>(
            id.getStringId());
  }

  SyncObservable(const T& value) : m_value(value) {
    JackRussell::TopicManager::getInstance()
        .createSyncTopic<std::pair<std::shared_ptr<T>, std::shared_ptr<T>>>(
            id.getStringId());
  }
...
#endif  // DOGBREEDS_POMERANIAN_SYNCOBSERVABLE_H